...
export default function SpendingList({ spendings, filtering, ordering, setSpendings }) {
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(false);

  useEffect(() => {
    fetch(`http://localhost:5000/spendings?currency=${encodeURIComponent(filtering)}&order=${encodeURIComponent(ordering)}`, {
      method: "GET",
